...
class TransactionGenerator:
    ...
    def __init__(
        self,
        trans_per_sec: int = 10,
        fraudulent_transactions_freq: int = 10,
    ...
    def generate_high_frequency_fraudulent_transactions(self) -> list[Transaction]:
        tx = self.generate_valid_transaction(user_id=random.randint(21000, 29999))
        transactions = []
        for i in range(random.randint(2, 4)):
            ...
            transactions.append(
                self.generate_valid_transaction(
                    timestamp=new_ts, user_id=tx.user_id, card_id=tx.card_id
                )
            )
        return transactions
    ...
    def generate_transactions(self):
        # Valid transactions
        threading.Thread(target=self.valid_transactions_thread, daemon=True).start()

        # Fraudulent thread: High frequency
        threading.Thread(
            target=self.fraudulent_transactions_thread,
        ...
        threading.Thread(
            target=self.fraudulent_transactions_thread,
            args=(self.generate_different_country_fraudulent_transactions,),
            daemon=True,
        ).start()
